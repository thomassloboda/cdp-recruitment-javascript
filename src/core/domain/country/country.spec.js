...
describe('Country', () => {
  it('should have a string name', () => {
    ...
    [undefined, null, '', ' '].forEach((name) => {
      expect(() => new Country(name)).toThrowError(
        new MissingRequiredParameterError('name')
      );
    });
    expect(() => new Country(1)).toThrowError(
  ...
  it('should have a valid people array', () => {
    ...
    // invalid cases
    expect(() => new Country('USA', null)).toThrowError(
      new MissingRequiredParameterError('people')
    );
    expect(() => new Country('USA', ['item'])).toThrowError(
      new WrongTypeParameterError('people', 'Person[]')
    );
  ...
  describe('toString', () => {
    it('should return the country name and people', () => {
      ...
      expect(country.toString()).toEqual({
        name: 'USA',
